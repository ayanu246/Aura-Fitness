...
# --- LOGIN & AUTO-RESTORE SYSTEM ---
if 'auth' not in st.session_state: st.session_state.auth = False

if not st.session_state.auth:
    st.title("🌙 Aura Elite")
    user = st.text_input("Athlete Username", placeholder="Enter to Save Progress")
...
for key, val in {"steps": 0, "water": 0, "exercise": 0, "active_group": "Global"}.items():
...
with t1:
    ...
    c1.markdown(f'<div class="stat-card"><p class="metric-title">Move</p><p class="metric-value" style="color:#ff2d55;">{st.session_state
