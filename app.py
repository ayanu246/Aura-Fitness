...
with t1:
    ...
    if st.button("CONNECT TO HEALTH CLOUD"):
        st.info("Initiating secure device handshake...")
        # Requests hardware permission (requires Add to Home Screen to work 100%)
        streamlit_js_eval(js_expressions="""
            DeviceMotionEvent.requestPermission().then(res => {
                if (res == 'granted') { window.alert('Hardware Handshake Verified'); }
            })
        """, key="hw_sync")
        ...
        st.toast("Progress Synchronized.")
...
with t3:
    ...
    # STRICT FILTER: Only show people in the selected group
    try:
...
with t4:
    ...
    if st.button("LOCK IN TEAM"):
        if new_g:
            ...
            supabase.table("aura_collab_tracker").upsert(p, on_conflict="username,group_name").execute()
            st.success(f"You are now competing in: {new_g}")
            st.rerun()
